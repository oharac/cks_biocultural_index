...
nr <- 20 ### number of rows for panel A
tx_plot_df <- cks_raw %>%
  full_join(tx_lbls) %>%
  arrange(desc(tx_lbl))
...
panelB <- ggdraw() +
  ...
  draw_plot(glb_p, x = .87, y = .6, width = w$glo, height = h$glo) +
  draw_label('Multiple\ncontinents\n(n = 4)', x = .873, y = .6, size = 11, hjust = 0, vjust = 1) +
  ...
  draw_plot(lgd, x = .5, y = .05, width = .15, height = .15)
...
plot_f <- here('fig1_cis_spp_continent.png')

map_w <- .8
plot_w <- 12; plot_h <- 5

fig1 <- ggdraw() +
  draw_plot(panelA, x = .01, y = 0, width = (1 - map_w), height = 1) +
  draw_plot(panelB, x = (1 - map_w), y = 0, width = map_w, height = 1) +
  draw_label('A', x = 0.00, y = .9, hjust = 0, vjust = 1, size = 16, fontface = 'bold') +
  draw_label('B', x = 0.25, y = .9, hjust = 0, vjust = 1, size = 16, fontface = 'bold')
